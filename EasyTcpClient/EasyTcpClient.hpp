...
public:
	...
	bool OnRun()
	{
		if (isRun())
		{
			...
			if (ret < 0)
			{
				printf("<socket=%d>select task finished 1\n", _sock);
				Close();
				return false;
			}
			if (FD_ISSET(_sock, &fdReads))
			{
				...
				if (-1 == RecvData(_sock))
				{
					printf("<socket=%d>select task finished 2\n", _sock);
					Close();
					return false;
				}
			}
...
#define RECV_BUFF_SZIE 10240
	...
	int RecvData(SOCKET cSock)
	{
		...
		if (nLen <= 0)
		{
			printf("<socket=%d>has disconnect server,task finished.\n", cSock);
			return -1;
		}
		//将收取到的数据拷贝到消息缓冲区
	...
	virtual void OnNetMsg(DataHeader* header)
	{
		switch (header->cmd)
		{
			...
			case CMD_ERROR:
			{
				printf("<socket=%d>recv server msg:CMD_ERROR,data len:%d\n", _sock, header->dataLength);
			}
			break;
			default:
			{
				printf("<socket=%d>recv server unknow msg,data len:%d\n", _sock, header->dataLength);
			}
		}
	}
